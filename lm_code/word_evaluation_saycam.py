...
from run_rnn_language_modeling import format_inputs, format_inputs_saycam


import sys
sys.path.append('/ext3/multimodal-baby')
...
_LSTM_SAYCAM_PAD_TOKEN = 0
_LSTM_SAYCAM_MASK_TOKEN = 5589
# _LSTM_SAYCAM_MASK_TOKEN = 1
_LSTM_SAYCAM_MAX_SEQ_LEN = 512
...
def get_sample_sentences(tokenizer, wordbank_file, wordbank_lang, tokenized_examples_file,
    ...
    # Get token ids.
    if type(tokenizer) == dict:
        for token in wordbank_tokens:
            token_id = tokenizer.get(token, tokenizer['<unk>'])
            if token_id != tokenizer['<unk>']:
    ...
    for line_count, line in enumerate(infile):
        ...
        for token, token_id, sample_sents in token_data:
            ...
            if len(token_indices) > 0:
                if args.model_type == 'lstm_saycam':
                    new_example = example.copy()
                    target_idx = random.choice(token_indices)
                    sample_sents.append((new_example, target_idx))
                else:
                    new_example = example.copy()
...
def prepare_tokenized_examples(tokenized_examples, tokenizer, model_type):
    ...
    if model_type == 'lstm_saycam':
        tokenized_examples_only = [ex_tok for ex_tok, _ in tokenized_examples]
        tensor_examples = [torch.tensor(e, dtype=torch.long) for e in tokenized_examples_only]
    else:    
    ...
    if model_type == 'lstm_saycam':
        ...
        input_ids = pad_sequence(tensor_examples, batch_first=True,
                                 padding_value=_LSTM_SAYCAM_PAD_TOKEN)
    else:
        input_ids = pad_sequence(tensor_examples, batch_first=True,
                                 padding_value=tokenizer.pad_token_id)
    labels = input_ids.clone().detach()
    if model_type == 'lstm_saycam':
        labels[labels == _LSTM_SAYCAM_PAD_TOKEN] = -100
        attention_mask = input_ids != _LSTM_SAYCAM_PAD_TOKEN
        # format_input cuts input lengths by 1
        # y_len = torch.sum(attention_mask.long(), dim=1) - 1
        y_len = torch.sum(attention_mask.long(), dim=1)
        # Target positions are shifted by 1
        target_positions = [target_idx - 1 for _, target_idx in tokenized_examples]
    else:
    ...
    elif model_type =='lstm_saycam':
        inputs = format_inputs_saycam(inputs, input_ids.shape[1], bidirectional=False)
        inputs["y_len"] = y_len
        inputs["target_positions"] = target_positions
    if torch.cuda.is_available():
        inputs["input_ids"] = inputs["input_ids"].cuda()
        if inputs.get("attention_mask", None):
...
def run_model(model, model_type, examples, batch_size, tokenizer):
    ...
    if len(examples) % batch_size != 0:
        ...
        if model_type == "lstm" or model_type == "bilstm":
            ...
            dummy_example = [0, 0, _LSTM_SAYCAM_PAD_TOKEN, 0, 0]
            while len(batches[-1]) % torch.cuda.device_count() != 0:
                batches[-1].append(dummy_example)
                num_dummies += 1
    # Run evaluation.
    ...
    with torch.no_grad():
        ...
        for batch_i in range(len(batches)):
            ...
            if model_type == "bert" or model_type == "gpt2": # Transformers
                ...
                # print(inputs["y_len"])
                _, _, logits, _ = model.language_model.calculate_ce_loss(inputs["input_ids"], inputs["y_len"], tokenwise=True)
            ...
            if model_type == 'lstm_saycam':
                target_indices_int = inputs["target_positions"]
                bool_indices = []
                index_list = [list(range(0, logits.shape[1]))]*logits.shape[0]
                target_indices = torch.tensor([np.array(li) == targ_idx for li, targ_idx in zip(index_list, target_indices_int)])
                # print("logit shape: ", logits.shape)
                # print(target_indices_int)
                # print(index_list)
                # print(index_list_bool)
            else:
...
def load_single_model(single_model_dir, model_type, config, tokenizer):
    ...
    if model_type == "lstm" or model_type == "bilstm":
        ...
        model = MultiModalLitModel.load_from_checkpoint(single_model_dir)
    # Load onto GPU.
    if torch.cuda.is_available():
        model = model.cuda()
...
def main(args):
    ...
    if args.model_type == "gpt2" or args.model_type == "bert":
        ...
        if args.model_type == 'lstm_saycam':
            max_seq_len = _LSTM_SAYCAM_MAX_SEQ_LEN
        else:
    ...
    # Load tokenizer.
    if args.model_type == 'lstm_saycam':
        with open(args.tokenizer) as wf:
            tokenizer = json.load(wf)
    else:
    ...
    if args.checkpoints is None or len(args.checkpoints) == 0:
        ...
        for root, dirs, files in os.walk(args.model_dir):
            if args.model_type == 'lstm_saycam':
                for file in files:
                    if "epoch=" in file or "step=" in file:
                        checkpoint = int(os.path.splitext(file)[0].split("=")[-1])
    ...
    for checkpoint in checkpoints:
        ...
        if args.model_type == 'lstm_saycam':
            single_model_dir = os.path.join(args.model_dir, "step={}.ckpt".format(checkpoint))
        else:
